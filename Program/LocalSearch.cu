...
void LocalSearch::loadINDIV_CUDA(Individual &indiv, int nbVehicles, int nbClients, int nbMoves, double penaltyCapacityLS, double penaltyDurationLS)
{
	std::vector<int> routeSizes(nbVehicles, 0);
	routeSizes[0] = indiv.chromR[0].size();
	for (int i = 1; i < nbVehicles; i++)
	{
		routeSizes[i] = indiv.chromR[i].size() + routeSizes[i - 1];
	}
...
void LocalSearch::run(Individual &indiv, double penaltyCapacityLS, double penaltyDurationLS)
{
	...
	// double n_g_time = 0, swap_start_time = 0;
	unsigned int *d_best_move = nullptr;
	unsigned int *d_best_v_index = nullptr;
	unsigned int *d_best_u_index = nullptr;
	unsigned int Best_move = 0, Best_v_index = 0, Best_u_index = 0;
	cudaMalloc(&d_best_move, sizeof(unsigned int));
	cudaMalloc(&d_best_v_index, sizeof(unsigned int));
	cudaMalloc(&d_best_u_index, sizeof(unsigned int));
	...
	for (loopID = 0; !searchCompleted; loopID++)
	{
		...
		loadINDIV_CUDA(indiv, params.nbVehicles, params.nbClients, nbMoves, penaltyCapacityLS, penaltyDurationLS);
		int emptyRouteIndex = -1;
		if (emptyRoutes.size() > 0)
			emptyRouteIndex = *emptyRoutes.begin();
		kernel1_CUDA<<<1, params.nbClients>>>(loopID, emptyRouteIndex, d_best_move, d_best_v_index, d_best_u_index);
		cudaDeviceSynchronize();
		cudaMemcpy(&Best_move, d_best_move, sizeof(unsigned int), cudaMemcpyDeviceToHost);
		cudaMemcpy(&Best_v_index, d_best_v_index, sizeof(unsigned int), cudaMemcpyDeviceToHost);
		cudaMemcpy(&Best_u_index, d_best_u_index, sizeof(unsigned int), cudaMemcpyDeviceToHost);
		if (Best_move != 0)
		{
			Node *nodeU = &clients[Best_u_index];
			Node *nodeV = &clients[Best_v_index];
		...
		// 	// Node_CUDA *nodeU_CUDA = (Node_CUDA *)malloc(sizeof(Node_CUDA));
		// 	// fill_node(nodeU_CUDA, nodeU);
...
LocalSearch::LocalSearch(Params &params) : params(params)
{
	...
	cudaMalloc(&d_clients, (params.nbClients + 1) * sizeof(Node_CUDA));
	cudaMalloc(&d_routes, params.nbVehicles * sizeof(Route_CUDA));
	cudaMalloc(&d_depots, params.nbVehicles * sizeof(Node_CUDA));
	cudaMalloc(&d_depotsEnd, params.nbVehicles * sizeof(Node_CUDA));
	cudaMalloc(&d_route_size, sizeof(int) * params.nbVehicles);
	cudaMalloc(&d_chrom_T, sizeof(int) * (params.nbClients + 1));

	Initialize_LocalSearch_CUDA<<<1, 1>>>(params.nbClients, params.nbVehicles, d_clients, d_routes, d_depots, d_depotsEnd, d_route_size, d_chrom_T);
	cudaDeviceSynchronize();
}
